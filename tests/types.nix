...
let
  ntypes = ninx.types;
  typeEq = type: val: type.check val && ntypes.expr.check val;
in
with ninx;
...
[
  ...
  (test "application" (typeEq ntypes.application (app (lambda "a" (var "a")) 1)))
  (test "import" (typeEq ntypes.import (importt ./default.nix)))
  (test "raw" (typeEq ntypes.raw (raw "x: x.a.b.c.d")))
]
